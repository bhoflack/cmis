...
(defn make-tar-dir
  ...
  (let [dir (tar-dir project)]
    ...
    (copy (file (:root project) "target" 
          ...
          (file dir "lib" (str (:name project) ".jar")))

    ; Binary
    (.mkdirs (file dir "bin"))
    (copy (file (:root project) "pkg" "tar" (:name project))
          (file dir  "bin" (:name project)))
    (.setExecutable (file dir "bin" (:name project)) true false)
