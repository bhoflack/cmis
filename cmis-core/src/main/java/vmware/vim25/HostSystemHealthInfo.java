
package vmware.vim25;

import java.util.ArrayList;
import java.util.List;
import javax.xml.bind.annotation.XmlAccessType;
import javax.xml.bind.annotation.XmlAccessorType;
import javax.xml.bind.annotation.XmlType;


/**
 * <p>Java class for HostSystemHealthInfo complex type.
 * 
 * <p>The following schema fragment specifies the expected content contained within this class.
 * 
 * <pre>
 * &lt;complexType name="HostSystemHealthInfo">
 *   &lt;complexContent>
 *     &lt;extension base="{urn:vim25}DynamicData">
 *       &lt;sequence>
 *         &lt;element name="numericSensorInfo" type="{urn:vim25}HostNumericSensorInfo" maxOccurs="unbounded" minOccurs="0"/>
 *       &lt;/sequence>
 *     &lt;/extension>
 *   &lt;/complexContent>
 * &lt;/complexType>
 * </pre>
 * 
 * 
 */
@XmlAccessorType(XmlAccessType.FIELD)
@XmlType(name = "HostSystemHealthInfo", propOrder = {
    "numericSensorInfo"
})
public class HostSystemHealthInfo
    extends DynamicData
{

    protected List<HostNumericSensorInfo> numericSensorInfo;

    /**
     * Gets the value of the numericSensorInfo property.
     * 
     * <p>
     * This accessor method returns a reference to the live list,
     * not a snapshot. Therefore any modification you make to the
     * returned list will be present inside the JAXB object.
     * This is why there is not a <CODE>set</CODE> method for the numericSensorInfo property.
     * 
     * <p>
     * For example, to add a new item, do as follows:
     * <pre>
     *    getNumericSensorInfo().add(newItem);
     * </pre>
     * 
     * 
     * <p>
     * Objects of the following type(s) are allowed in the list
     * {@link HostNumericSensorInfo }
     * 
     * 
     */
    public List<HostNumericSensorInfo> getNumericSensorInfo() {
        if (numericSensorInfo == null) {
            numericSensorInfo = new ArrayList<HostNumericSensorInfo>();
        }
        return this.numericSensorInfo;
    }

}
